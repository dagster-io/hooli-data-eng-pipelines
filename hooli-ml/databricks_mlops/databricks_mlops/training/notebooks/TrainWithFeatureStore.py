...
# ``databricks_mlops/resources/model-workflow-resource.yml``
#
# Parameters:
...
dbutils.widgets.text(
    "experiment_name",
    f"/dev-databricks_mlops-experiment",
...
dbutils.widgets.text(
    "model_name", "dev.databricks_mlops.databricks_mlops-model", label="Full (Three-Level) Model Name"
)

# Pickup features table name
dbutils.widgets.text(
    "pickup_features_table",
    "dev.databricks_mlops.trip_pickup_features",
...
dbutils.widgets.text(
    ...
    "dev.databricks_mlops.trip_dropoff_features",
...
def rounded_unix_timestamp(dt_col, num_minutes=15):
    """
    Ceilings datetime dt_col to interval num_minutes, then returns the unix timestamp.
    """
    nsecs = (F.minute(dt_col) * 60 + F.second(dt_col))
    interval = 60 * num_minutes
    delta = (F.ceil(nsecs / interval) * interval - nsecs)
    # Add delta seconds to the timestamp
    return F.unix_timestamp(dt_col) + delta

#rounded_unix_timestamp_udf = F.udf(rounded_unix_timestamp, IntegerType())
#spark.udf.register("rounded_unix_timestamp", rounded_unix_timestamp, IntegerType())
