...
The databricks CLI bundles top level is defined by file `databricks_mlops/databricks.yml`.
...
ML Resource Configurations in this directory:
 - model workflow (`databricks_mlops/resources/model-workflow-resource.yml`)
 - batch inference workflow (`databricks_mlops/resources/batch-inference-workflow-resource.yml`)
 - monitoring resource and workflow (`databricks_mlops/resources/monitoring-resource.yml`)
 - feature engineering workflow (`databricks_mlops/resources/feature-engineering-workflow-resource.yml`)
 - model definition and experiment definition (`databricks_mlops/resources/ml-artifacts-resource.yml`)
...
If you want to use different catalog names, please update the `targets` declared in the `databricks_mlops/databricks.yml` and `databricks_mlops/resources/ml-artifacts-resource.yml` files.
...
|-------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|----------------------|-------------------------------------|------------------------------------------------|
| dev         | The `dev` deployment target is used by ML engineers to deploy ML resources to development workspace with `dev` configs. The config is for ML project development purposes.                                                           | dev workspace        | dev-databricks_mlops-model     | /dev-databricks_mlops-experiment     |
| staging     | The `staging` deployment target is part of the CD pipeline. Latest master content will be deployed to staging workspace with `staging` config.                                                             | staging workspace    | staging-databricks_mlops-model | /staging-databricks_mlops-experiment |
| prod        | The `prod` deployment target is part of the CD pipeline. Latest release content will be deployed to prod workspace with `prod` config.                                                                      | prod workspace       | prod-databricks_mlops-model    | /prod-databricks_mlops-experiment    |
| test        | The `test` deployment target is part of the CI pipeline. For changes targeting the master branch, upon making a PR, an integration test will be triggered and ML resources deployed to the staging workspace defined under `test` deployment target. | staging workspace    | test-databricks_mlops-model    | /test-databricks_mlops-experiment    |
...
2. Complete the `TODO` in `databricks_mlops/databricks.yml` to add the dev workspace URI under `targets.dev.workspace.host`.
...
```
Following this, update the batch_inference_job base parameters in `databricks_mlops/resources/batch-inference-workflow-resource.yml` to pass
...
To get started, open `databricks_mlops/resources/batch-inference-workflow-resource.yml`.  The file contains the ML resource definition of a batch inference job, like:
...
resources:
  jobs:
    batch_inference_job:
      name: ${bundle.target}-databricks_mlops-batch-inference-job
      tasks:
...
The example above defines a Databricks job with name `${bundle.target}-databricks_mlops-batch-inference-job`
that runs the notebook under `databricks_mlops/deployment/batch_inference/notebooks/BatchInference.py` to regularly apply your ML model for batch inference. 
...
We specify a `batch_inference_job` under `resources/jobs` to define a databricks workflow with internal key `batch_inference_job` and job name `{bundle.target}-databricks_mlops-batch-inference-job`.
...
`test-databricks_mlops-batch-inference-job`. During the deployment of the `staging` environment config, the job name will be
`staging-databricks_mlops-batch-inference-job`.
