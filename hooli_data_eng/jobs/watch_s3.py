...
from hooli_data_eng.resources.sensor_file_managers import LocalFileSystem, s3FileSystem
...
sensor_resources = {
    "LOCAL": LocalFileSystem(base_dir=file_relative_path(__file__, ".")),
    "PROD":  s3FileSystem(region_name="us-west-2",s3_bucket="hooli-demo"),
    "BRANCH": s3FileSystem(region_name="us-west-2",s3_bucket="hooli-demo-branch")
}
...
def watch_s3_sensor(context, fs: sensor_resources[get_env()]):
    last_mtime = float(context.cursor) if context.cursor else 0
    env = get_env()
    try:
        last_updated = fs.get_mtime("customers.txt")
