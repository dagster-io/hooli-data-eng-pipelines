...
from dagster import AssetCheckResult, asset_check, AssetKey, DagsterEventType, EventRecordsFilter
import jsonschema
from sklearn.linear_model import LinearRegression
import numpy as np
from dagster import build_last_update_freshness_checks, build_time_partition_freshness_checks, AssetKey, build_column_schema_change_checks
from dagster import AssetKey, FreshnessPolicy, AssetCheckSpec, asset_check
...
def handle_freshness_check(check: dict):
    ...
    for asset in assets:
        ...
        if "maximum_lag_minutes_by_partition" in check:
            policy = FreshnessPolicy(maximum_lag_minutes=check["maximum_lag_minutes_by_partition"], cron_schedule="* * * * *")  # cron required for partition
...
def apply_filters(df: pd.DataFrame, where: str = None, group_by: str = None) -> Dict[str, pd.DataFrame]:
    if where:
        df = df.query(where)
    if group_by:
...
def evaluate_anomaly_detection(context, asset: str, metric: str, history: int, threshold: float, latest_value: float) -> AssetCheckResult:
    ...
    if len(historical_values) < history:
        return AssetCheckResult(passed=True, metadata={"note": "Not enough history", metric: latest_value})
...
def evaluate_static_threshold(latest_value: float, min_value: float = None, max_value: float = None) -> AssetCheckResult:
    passed = True

    if isinstance(latest_value, datetime):
        latest_value = MetadataValue.timestamp(latest_value)
    else:
        latest_value = float(latest_value)  # For numeric consistency
    ...
    if isinstance(max_value, str):
        max_value = datetime.fromisoformat(min_value)
    ...
    return AssetCheckResult(
        passed=passed,
        metadata={"min_threshold": min_value, "max_threshold": max_value, "latest": latest_value},
    )

def evaluate_percent_delta(context, asset: str, metric: str, threshold_pct: float, latest_value: float) -> AssetCheckResult:
    records = context.instance.get_event_records(
        EventRecordsFilter(
            event_type=DagsterEventType.ASSET_CHECK_EVALUATION,
            asset_key=AssetKey(asset),
    ...
    if not records:
        return AssetCheckResult(passed=True, metadata={"note": "No prior value", metric: latest_value})

    prev_value = records[0].asset_check_evaluation.metadata.get(metric)
    delta_pct = abs(latest_value - prev_value) / prev_value * 100 if prev_value != 0 else float("inf")
    passed = delta_pct <= threshold_pct
    return AssetCheckResult(
        passed=passed,
        metadata={"previous": prev_value, "latest": latest_value, "delta_pct": delta_pct, "threshold_pct": threshold_pct},
    )

def evaluate_prediction_check(context, asset: str, metric: str, history: int, confidence: float, latest_value: float) -> AssetCheckResult:
    records = context.instance.get_event_records(
        EventRecordsFilter(
            event_type=DagsterEventType.ASSET_CHECK_EVALUATION,
            asset_key=AssetKey(asset),
    ...
    return AssetCheckResult(
        passed=passed,
        metadata={"predicted": prediction, "lower": lower, "upper": upper, "latest": latest_value, "confidence": confidence},
    )

def compute_group_metric(df: pd.DataFrame, metric: str) -> float:
    import re
...
def evaluate_grouped_static_threshold(df_map: Dict[str, pd.DataFrame], metric: str, min_value: float, max_value: float, allowed_failures: int) -> AssetCheckResult:
    failures = []
    metadata = {}
    ...
    return AssetCheckResult(
        passed=passed,
        metadata={"group_results": metadata, "failures": failures, "allowed_failures": allowed_failures},
    )

def evaluate_grouped_percent_delta(context, asset: str, df_map: Dict[str, pd.DataFrame], metric: str, threshold_pct: float, allowed_failures: int) -> AssetCheckResult:
    failures = []
    metadata = {}
...
def evaluate_grouped_prediction_check(context, asset: str, df_map: Dict[str, pd.DataFrame], metric: str, history: int, confidence: float, allowed_failures: int) -> AssetCheckResult:
    failures = []
    metadata = {}
    z = {0.90: 1.645, 0.95: 1.96, 0.99: 2.576}.get(confidence, 1.96)
...
def evaluate_grouped_anomaly_detection(context, asset: str, df_map: Dict[str, pd.DataFrame], metric: str, threshold: float, history: int, allowed_failures: int) -> AssetCheckResult:
    failures = []
    metadata = {}
...
def evaluate_grouped_check(context, asset: str, df_map: Dict[str, pd.DataFrame], check_type: str, metric: str, history: int, threshold: float, threshold_pct: float, confidence: float, allowed_failures: int) -> AssetCheckResult:
    if metric.startswith("distribution_change:"):
        ...
        return evaluate_grouped_static_threshold(df_map, metric, threshold, threshold, allowed_failures)
    elif check_type == "percent_delta":
        return evaluate_grouped_percent_delta(context, asset, df_map, metric, threshold_pct, allowed_failures)
    elif check_type == "predicted_range":
        return evaluate_grouped_prediction_check(context, asset, df_map, metric, history, confidence, allowed_failures)
    elif check_type == "anomaly_detection":
        return evaluate_grouped_anomaly_detection(context, asset, df_map, metric, threshold, history, allowed_failures)
    else:
...
def generate_asset_check_function(parsed_check: Dict[str, Any]):
    ...
    print(f"📎 Registering check: {check_name}, AssetKey: {asset}")

    @asset_check(name=check_name, asset=AssetKey(asset.split(".")) if "." in asset else AssetKey(asset))
    def dynamic_check(context, asset_data):
        ...
        if group_by:
            ...
            if metric == "num_rows":
                ...
                result = AssetCheckResult(passed=True, metadata={column: current_dist.to_dict(), metric: value})
            ...
            if result is None and value is not None:
                ...
                if check_type == "anomaly_detection":
                    result = evaluate_anomaly_detection(context, asset, metric, history, threshold, value)
                elif check_type == "static_threshold":
                    ...
                    result = evaluate_prediction_check(context, asset, metric, history, confidence, value)
                else:
                    raise ValueError(f"Unsupported check type: {check_type}")

            if result is not None and value is not None and not metric.startswith("distribution_change"):
