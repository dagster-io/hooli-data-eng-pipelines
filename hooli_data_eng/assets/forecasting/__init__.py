...
# Define SourceAsset for continent_stats
continent_stats_source = SourceAsset(key=AssetKey(["continent_stats"]))

# This databricks pipes asset only runs in prod, see utils/my_other_external_databricks_script.py
# The dependency on continent_stats is not a real dependency since the script does not rely
# or use that upstream asset, it is used here for illustrative purposes
@asset(
    deps=[continent_stats_source],
    compute_kind="databricks",
    tags={**StorageKindTagSet(storage_kind="databricks")},
)
    ...
    task = jobs.SubmitTask.from_dict(
        {
            ...
            "task_key": "dagster-launched",
            "spark_python_task": {
                "python_file": "dbfs:/FileStore/my_pipes_examples/my_other_external_databricks_script.py",
                "source": jobs.Source.WORKSPACE,
            },
        }
    )
