...
sensor_resources = {
    "LOCAL": {
        "fs": local_fs.configured({"base_dir": file_relative_path(__file__, ".")})
    },
    "PROD": {
        "fs": s3_fs.configured({"region_name": "us-west-2", "s3_bucket": "hooli-demo"})
    },
    "BRANCH": {
        "fs": s3_fs.configured(
...
# test it out
with build_resources(sensor_resources[get_env()]) as resources:
