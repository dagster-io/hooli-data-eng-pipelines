...
import tiktoken
from hooli_ai.resources.github import GithubResource
from hooli_ai.resources.pinecone import PineconeResource
from hooli_ai.resources.scraper import SitemapScraper
...
def github_discussions_embeddings(
    ...
    index.upsert(
        vectors=zip(
            [str(i) for i in range(len(split_texts))],
            all_embeddings,
            metadata,
        ),
        **namespace_kwargs,
...
def docs_embedding(
    ...
    pinecone.create_index("dagster-knowledge", dimension=1536)
    index, namespace_kwargs = pinecone.get_index(
        "dagster-knowledge", namespace="dagster-docs"
    )
