...
from hooli_ai.resources.github_gql_queries import (
...
class GithubResource(dg.ConfigurableResource):
    ...
    def get_issues(self, start_date="2023-01-01", end_date="2023-12-31") -> list[dict]:
        issues_query_str = GITHUB_ISSUES_QUERY.replace(
            "START_DATE", start_date
        ).replace("END_DATE", end_date)
        return self._fetch_results(issues_query_str, "issues")

    def get_discussions(
        self, start_date="2023-01-01", end_date="2023-12-31"
    ) -> list[dict]:
        discussion_query_str = GITHUB_DISCUSSIONS_QUERY.replace(
            "START_DATE", start_date
        ).replace("END_DATE", end_date)
    ...
    def convert_discussions_to_documents(
        ...
        for item in items:
            try:
                ...
                log.error(
                    f"Error processing discussion #{item.get('number', 'unknown')}:"
                )
